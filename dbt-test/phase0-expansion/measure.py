"""Recompute phase0-expansion whole-line savings from reviewed labels; no filter rules."""
import hashlib
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parent
CAPTURES = ROOT.parent / 'captures' / 'fusion-expansion'
MEMBERS = {'run': ['run-no-selection', 'run-db-error', 'run-compile-error'],
           'test': ['test-warning'],
           'build': ['build-mixed-skip', 'build-larger-terminal']}
...
def main():
    ...
    result = dict(method='ceil(UTF-8 bytes/4); byte-preserving line deletion; no ANSI normalization or newline rewriting',
                  scope='Expansion corpus (edge cases + larger PTY run), small DuckDB playground; not representative production cohorts',
                  ansi_crlf='PTY capture: splitlines records carry CRLF+ANSI; kept lines emitted byte-for-byte; no stripping',
                  held_out='captures/fusion/test-restored.* not read or measured',
                  samples=samples, cohorts=cohorts)
