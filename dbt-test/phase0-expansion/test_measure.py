"""Tests for phase0-expansion measure.py: estimator rounding, stream join, savings math."""
import unittest
...
class TestJoinStreams(unittest.TestCase):
    ...
    def test_pty_ansi_crlf_lines_pass_through_unchanged(self):
        raw = b'\x1b[32m\x1b[1m    Passed\x1b[0m [  0.00s] test  x\x1b[0m\r\n' \
              b'Processed: 14 models | 20 tests\r\n'
        self.assertEqual(join_streams(raw, b''), raw)
        self.assertEqual(len(raw.splitlines(keepends=True)), 2)   # one record per CRLF line
        kept = raw.splitlines(keepends=True)[1]
        self.assertEqual(kept.endswith(b'\r\n'), True)
